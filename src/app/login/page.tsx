  ...
  return (
    <div className="min-h-screen bg-[#E8EDE6] flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <motion.div 
        ...
        className="absolute top-12 left-12 h-24 w-24 rounded-sm border-2 border-[#0F1A0F] bg-[#D4A843] shadow-[4px_4px_0px_#0F1A0F] rotate-12 opacity-60" 
      />
      <motion.div 
        initial="initial"
        animate="animate"
        variants={floatingVariants(1)}
        className="absolute bottom-16 right-16 h-32 w-32 rounded-sm border-2 border-[#0F1A0F] bg-[#3B6B4A] shadow-[4px_4px_0px_#0F1A0F] -rotate-6 opacity-40" 
      />
      <motion.div 
        initial="initial"
        animate="animate"
        variants={floatingVariants(2)}
        className="absolute top-1/3 right-12 h-16 w-16 rounded-sm border-2 border-[#0F1A0F] bg-[#8B4A2B] shadow-[3px_3px_0px_#0F1A0F] rotate-3 opacity-50" 
      />
      <motion.div 
        initial="initial"
        animate="animate"
        variants={floatingVariants(0.5)}
        className="absolute bottom-1/3 left-16 h-12 w-12 rounded-sm border-2 border-[#0F1A0F] bg-[#F5F8F4] shadow-[3px_3px_0px_#0F1A0F] -rotate-12 opacity-70" 
      />
        ...
        <div className="rounded-md border-2 border-[#0F1A0F] bg-[#F5F8F4] p-10 shadow-[8px_8px_0px_#0F1A0F] text-center space-y-8">
          ...
          <div className="space-y-6">
            ...
            <motion.button
              ...
              onClick={handleLogin}
              disabled={isLoading}
              className={`w-full inline-flex items-center justify-center gap-3 rounded-md border-2 border-[#0F1A0F] px-6 py-4 text-xl font-black uppercase italic shadow-[5px_5px_0px_#0F1A0F]
                ${isLoading 
                  ? "bg-[#5A6E5A] text-[#F5F8F4]/50 cursor-not-allowed shadow-none translate-x-0.5 translate-y-0.5 grayscale" 
                  : "bg-[#3B6B4A] text-[#F5F8F4] hover:shadow-[7px_7px_0px_#0F1A0F] hover:-translate-x-px hover:-translate-y-px active:shadow-[2px_2px_0px_#0F1A0F] active:translate-x-0.5 active:translate-y-0.5"
                }`}
            >
              {isLoading ? (
                <>
